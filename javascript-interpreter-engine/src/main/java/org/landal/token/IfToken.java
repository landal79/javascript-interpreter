package org.landal.token;

/**
 * Questa classe rappresenta la categoria lessicale 'if'.
 * @author  Landini Alex
 * @version 1.0
 */
public class IfToken extends KeywordToken implements IIfToken{
    
    /**
     * Costruttore di IfToken.
     */
    public IfToken(){}
    
     /**
     * Questo metodo mi restituisce una stringa che identifica il tipo di token.
     * @return  String   identificatore del tipo di token.
     */
    public String getID(){ return "if";}
    
    /**
     * Fornisce una rappresentazione esterna del token.
     */
    public String toString (){
        return "if";
    }
    
}
