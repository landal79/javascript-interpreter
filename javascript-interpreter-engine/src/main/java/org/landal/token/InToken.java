package org.landal.token;

/**
 * Questa classe rappresenta la categoria lessicale 'in'.
 * @author  Landini Alex
 * @version 1.0
 */
public class InToken extends KeywordToken implements IInToken{
    
    /**
     * Costruttore di InToken.
     */
    public InToken(){}
    
     /**
     * Questo metodo mi restituisce una stringa che identifica il tipo di token.
     * @return  String   identificatore del tipo di token.
     */
    public String getID(){ return "in";}
    
     /**
     * Fornisce una rappresentazione esterna del token.
     */
    public String toString (){
        return "in";
    }
    
}
