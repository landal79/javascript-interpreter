package org.landal.token;

/**
 * Questa classe rappresenta la categoria lessicale 'else'.
 * @author  Landini Alex
 * @version 1.0
 */
public class ElseToken extends KeywordToken implements IElseToken{
    
    /**
     * Costruttore di ElseToken
     */
    public ElseToken(){}
    
     /**
     * Questo metodo mi restituisce una stringa che identifica il tipo di token.
     * @return  String   identificatore del tipo di token.
     */
    public String getID(){ return "else";}
    
    
     /**
     * Fornisce una rappresentazione esterna del token.
     */
    public String toString (){
        return "else";
    }
    
}
