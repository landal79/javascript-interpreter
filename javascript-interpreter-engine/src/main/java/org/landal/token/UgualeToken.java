package org.landal.token;

/**
 * Questa classe rappresenta la categoria lessicale '='.
 * @author  Landini Alex
 * @version 1.0
 */
 public class UgualeToken extends SimboliToken implements IUgualeToken{

    /**
     * Costruttore di UgualeToken.
     */
     public UgualeToken(){}
     
      /**
     * Questo metodo mi restituisce una stringa che identifica il tipo di token.
     * @return  String   identificatore del tipo di token.
     */
    public String getID(){ return "=";}

    /**
     * Questo metodo fornisce la rappresentazione esterna del token.
     */
     public String toString (){ return "="; }
     
 }