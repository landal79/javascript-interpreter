package org.landal.token;

/**
 * Classe che rappresenta la categoria lessicale '{'.
 * @author  Landini Alex
 * @version 1.0
 */
public class OpenParGToken extends SimboliToken implements IOpenParGToken{
    
    /**
     * Costruttore di OpenParGToken
     */
    public OpenParGToken() {}
    
     /**
     * Questo metodo mi restituisce una stringa che identifica il tipo di token.
     * @return  String   identificatore del tipo di token.
     */
    public String getID(){ return "{";}
    
    /**
     * Fornisce una rappresentazione esterna del token.
     */
    public String toString (){
       return "{";
    }
    
}

